...
class DatabaseNotificationObserver {
  async update(event) {
    // Notifications are shown by employee_id or role_id. One saved with
    // neither would be invisible to everyone, so say so instead of saving it.
    if (!event.employeeId && !event.roleId) {
      console.warn(`[notifications] "${event.title}" has no employeeId or roleId, so nobody could see it. Not saved.`);
      return;
    }
    await db.query(
